			...
			<main className={styles.main}>
				...
				<div>
					<div className={styles.card}>
						...
						<button className={styles.button} onClick={() => getGraphQl()}>
							Fetch
						</button>
						{products.map((p) => {
							return (
								<div key={p.node.id} className={styles.listItem}>
									...
									<p>
										{parseFloat(
											p.node.priceRangeV2.minVariantPrice.amount
										).toLocaleString("en-US", {
											style: "currency",
											currency:
												p.node.priceRangeV2.minVariantPrice.currencyCode,
										})}
									</p>
								</div>
							)
					...
					<div className={styles.card}>
						...
						<button className={styles.button} onClick={getApi}>
							Fetch
						</button>
						<h2>{message}</h2>
					</div>
